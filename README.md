...
- **Do not keep `node_modules` inside this package** when linking with `file:` from an app: a nested `react-native` will confuse Metro/codegen. Use hoisted peers only; delete `react-native-vungle/node_modules` if it appears. In the host app, add a Metro `resolver.blockList` regex for `node_modules/react-native-vungle/node_modules/react-native/` if needed.
